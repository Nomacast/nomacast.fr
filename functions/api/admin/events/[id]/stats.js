// functions/api/admin/events/[id]/stats.js
// GET /api/admin/events/:id/stats  →  Stats agrégées d'un event (vue admin Nomacast complète).
//
// Protégée par Basic Auth via functions/api/admin/_middleware.js (héritage).
// Retourne un JSON structuré : event meta, summary KPIs, timeline (drop-off), per_invitee, geography, top_chatters.
//
// Pour la vue admin client (filtrée), voir functions/api/event-admin/[token]/stats.js (livraison ultérieure).
//
// Marqueur : nomacast-analytics-stats-endpoint-v1
...
export const onRequestGet = async ({ params, env }) => {
  try {
    ...
    // 1. Récupération de l'event
    const event = await env.DB.prepare(`
      SELECT id, slug, title, client_name, scheduled_at, duration_minutes, audience_estimate,
             status, access_mode, requires_registration, client_organization_name,
    ...
    // 2. Summary KPIs — agrégations principales en queries parallèles
    const [
      inviteesAgg,
      ...
      // Invitees ayant cliqué au moins une fois (= row dans visits)
      env.DB.prepare(`
        SELECT COUNT(DISTINCT invitee_id) AS n
        FROM visits WHERE event_id = ? AND invitee_id IS NOT NULL
      `).bind(eventId).first(),

      // Invitees présents au live (= row dans presence_history)
      env.DB.prepare(`
        SELECT COUNT(DISTINCT invitee_id) AS n
        FROM event_presence_history WHERE event_id = ? AND invitee_id IS NOT NULL
      `).bind(eventId).first(),

      // Public unique viewers (page publique sans inscription, identifiés par anon_key)
      env.DB.prepare(`
        SELECT COUNT(DISTINCT anon_key) AS n
        FROM visits WHERE event_id = ? AND page_kind = 'public' AND anon_key IS NOT NULL
      `).bind(eventId).first(),
      ...
      // Chaque ping ≈ 30s de présence, on compte les pings par viewer puis moyenne
      env.DB.prepare(`
    ...
    // 3. Timeline complète — courbe de présence par tranches de 30s
    ...
    // 4. Per-invitee : détail engagement par invité
    const perInviteeRes = await env.DB.prepare(`
      SELECT
        ...
        i.invited_at,
        i.last_seen_at,
        (SELECT MIN(visited_at) FROM visits WHERE invitee_id = i.id) AS first_visit_at,
        (SELECT MAX(visited_at) FROM visits WHERE invitee_id = i.id) AS last_visit_at,
        (SELECT COUNT(*) FROM visits WHERE invitee_id = i.id) AS visits_count,
    ...
    // 5. Géographie (top 10 pays)
    const geoRes = await env.DB.prepare(`
      SELECT country_code, COUNT(*) AS count
      FROM visits
      WHERE event_id = ? AND country_code IS NOT NULL
    ...
    // 6. Sources de trafic (referrer agrégé par domaine)
    ...
    // 7. Top participants chat (qui parle le plus, sur messages approuvés)
    ...
    console.error('[admin/events/id/stats]', err);
