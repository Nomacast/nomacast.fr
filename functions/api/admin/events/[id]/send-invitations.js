...
import { buildInvitationEmail } from '../../_invitation-email.js';
...
export const onRequestPost = async ({ params, env }) => {
  ...
  // Charger les invités non-envoyés
  ...
  // Construction des payloads + chunking par lots de 100
  ...
  for (let i = 0; i < payloads.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        // On considère tout le chunk comme failed
        chunk.forEach(p => {
      ...
      // Resend renvoie {data: [{id}, {id}, ...]} pour batch
      // Marquer les invitees comme envoyés
