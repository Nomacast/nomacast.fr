// functions/i/[token].js
// GET /i/:token  →  Page participant via magic link.
//
// - Vérifie le token dans la table invitees
// - Trace last_seen_at (best effort)
// - Render HTML server-side selon event.status : draft / live / ended
...
export const onRequestGet = async ({ params, request, env }) => {
  ...
  if (!row) {
    return htmlResponse(renderErrorPage(
      ...
      'Ce lien d\'invitation n\'est pas valide, a expiré, ou a été révoqué. Contactez l\'organisateur de l\'événement pour obtenir un nouveau lien.'
    ), 404);
  }
  ...
  const invitee = {
    ...
    invited_at: row.invited_at, last_seen_at: row.last_seen_at
  };

  // Tracking last_seen_at (best effort)
  try {
    await env.DB.prepare('UPDATE invitees SET last_seen_at = ? WHERE id = ?')
  ...
  // (lazy : déclenché par le 1er GET après scheduled_at, pas besoin de cron)
  if (event.status === 'draft' && event.scheduled_at) {
    ...
    if (!isNaN(scheduledMs) && scheduledMs <= Date.now()) {
      try {
        ...
        console.error('[i/token] auto go-live failed', err);
      }
    }
  }
...
function renderEndedPage(event, invitee, token) {
  ...
  return htmlShell({
    ...
    bodyScript: buildLivePageScript({
      ...
      chatMessagesUrl: `/api/chat/${encodeURIComponent(event.slug)}/messages`,
      // nomacast-cta-ended-persist-v1 : on passe les params CTA même en ended
      ctaActiveUrl: hasCta ? `/api/chat/${encodeURIComponent(event.slug)}/cta/active` : null,
      hasCta,
...
<style>
  *,*::before,*::after { box-sizing: border-box; }
  ...
  /* HERO */
  .hero {
    background: ${color}; padding: 42px 24px;
  ...
  .state-ended { background: rgba(0,0,0,0.2); }
  ...
  .card-value {
    font-size: 15px; color: #0f172a; font-weight: 600; line-height: 1.4;
  }
  ...
  /* Placeholder viewer (live) */
  .placeholder-viewer {
    background: #ffffff; border: 1px solid #e2e8f0; border-radius: 14px;
    padding: 60px 32px; margin: 0 0 24px;
  ...
  .placeholder-title {
    font-size: 22px; font-weight: 700; margin: 0 0 8px; color: #0f172a;
  }
  .placeholder-text {
  ...
  /* Mobile : un peu de min-height visuel pour ne pas avoir un chat ridiculement petit */
  ...
  .footer-baseline {
    margin-top: 10px; color: #475569; font-size: 13px; font-style: italic;
  }
  .footer-separator {
    display: flex; gap: 8px; justify-content: center; margin: 22px 0;
  }
  .footer-separator span {
    width: 6px; height: 2px; background: #cbd5e1; display: inline-block;
  }
  .footer-blurb {
    color: #64748b; font-size: 13px; line-height: 1.6; margin-bottom: 18px;
  }
  .footer-links {
...
(function () {
  ...
  // après la fin de l'event (utile en replay : "demandez un devis", "ressources", etc.)
  // ============================================================
...
// Helpers Agenda
// ============================================================
function buildAgendaUrls(event, token) {
  const SITE_URL = 'https://nomacast.fr';
  const chatLink = `${SITE_URL}/i/${token}`;
