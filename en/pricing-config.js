const TVA = 1.20;
const PLATFORMS = ["YouTube","LinkedIn","Facebook","Twitch","Vimeo","Teams","Zoom","Custom RTMP","Other"];

// Partner codes: fetched on-the-fly via /api/validate-code (Cloudflare Pages Function)
// The object is populated dynamically after server-side validation, never in clear text in HTML.
const PARTNER_CODES = {};
// Cache of pretty display names (returned by the API), indexed by internal code
const PARTNER_DISPLAY_NAMES = {};

const CONFIG = {
  base: 1500,
  durations: {
    half:    { price: 1500, label: "Half-day", joursEq: 1 },
    full:    { price: 1750, label: "Full day", joursEq: 1 },
    "2days": { price: 2500, label: "2 days",   joursEq: 2 },
    "3days": { price: 3250, label: "3 days",   joursEq: 3 }
  }
};

// Discount multiplier per duration, based on the delta between "full" rate (1750/day) and actual base.
// On half-day and full day: no delta, mult = 1.0.
// On 2 days: (1750 × 2) / 2500 = 1.4. On 3 days: (1750 × 3) / 3250 ≈ 1.615.
// Effect: the effective discount is more generous on multi-day, exponential with duration.
const REMISE_MULT = {
  half:   1.0,
  full:   1.0,
  "2days": (1750 * 2) / 2500,
...
// Tiered savings: delta between "full" rate (€1,750/day) and the actual invoiced base.
// This discount is implicit in the tiered pricing (the client is already paying it via the reduced base),
// but we make it visible in the "Partner discount" banner to highlight the generosity
// of multi-day pricing. Shown only when a partner code is active.
//   - Half-day: 0 (no full-day notion)
//   - Full day: 0 (€1,750 invoiced for 1 day, no delta)
//   - 2 days: 1,750 × 2 - 2,500 = €1,000
//   - 3 days: 1,750 × 3 - 3,250 = €2,000
...
// List of options offered by the configurator. dayMultiplied = price multiplied by duration.
// dayMultiplied: price ×1.6 on 2d, ×2.1 on 3d+ (tiered pricing)
// 5g and cadreur_hf have their own price grid by duration (PRICE_5G_BY_DUR / PRICE_CADREUR_HF_BY_DUR)
const OPTIONS = [
  { id:"stream",     label:"Live broadcast",                desc:"To one or several platforms of your choice: YouTube, LinkedIn, Teams, Zoom…",  price:250, platforms:true },
  { id:"son",        label:"Sound kit",                     desc:"If your venue isn't equipped with sound: sound technician, wireless mics and speaker",   price:750, dayMultiplied:true },
  { id:"duplex",     label:"Remote speaker",                desc:"A participant who isn't on site joins via HD video, integrated directly into the picture", price:250, montageH:3 },
  { id:"reperage",   label:"Site survey",                   desc:"Technical visit before the event to anticipate constraints (lighting, sound, access, electricity)", price:350 },
  { id:"veille",     label:"Day-before setup",              desc:"Setup the night before to start day one stress-free. Île-de-France rate, provinces and international quoted separately.",  price:650 },
  { id:"cam_sup",    label:"Additional camera",             desc:"To film the audience, room reactions, or a second space (networking, workshop…)", price:250, dayMultiplied:true, montageH:3 },
  { id:"lumiere",    label:"Lighting kit",                  desc:"For high-quality lighting of your speakers and stage: dedicated lighting technician, fixtures and stage dressing",   price:750, dayMultiplied:true },
  { id:"ecran",      label:"Confidence monitor for speakers", desc:"The speaker can see what's being broadcast live (useful to follow slides or see remote guests)", price:150, dayMultiplied:true, montageH:3 },
  { id:"rush4k",     label:"Raw camera files (4K rushes)",  desc:"All the original video files from each camera, delivered on a drive. Useful if you plan to repurpose the content.", price:250 },
  { id:"5g",         label:"5G backup link",                desc:"Standalone internet network to secure the stream if the venue's WiFi is insufficient or unreliable", price:350, montageH:3 },
  { id:"montage_tc", label:"Chapter editing",               desc:"Final video split into key moments at the timecodes you provide (precise time markers)", price:300 },
  { id:"cadreur_hf", label:"Camera with operator (wireless feed)", desc:"A professional camera operator with a wireless camera, to follow the action in motion: film the audience, speakers moving around the room, or a second space in parallel (networking, workshop, backstage).", price:850, outsidePartner:true, montageH:3 }
];

// Tiered price grids per duration for options that don't follow the standard multiplier.
// 5G: linear tiered price (the router is shared, only data plans are added pro rata).
const PRICE_5G_BY_DUR       = { half: 350, full: 350, "2days": 500, "3days": 650 };
// Wireless camera operator: €850/day pure linear (no tiering, the freelance operator bills per day).
const PRICE_CADREUR_HF_BY_DUR = { half: 850, full: 850, "2days": 1700, "3days": 2550 };

// ═══ Post-event content — Step 04 ═══
// Outside partner mechanics: flat rate, added as-is to the total.
// Best-of varies with event duration (editing proportional to filmed material).
const ADDON_PRICES = {
...
// Technical kit details for add-ons, shown in "Tech view"
const ADDON_MATERIEL = {
  bestof: [
    "1× Dedicated director/editor on site",
    "1× Panasonic GH5 camera",
    "1× DJI RS3 stabiliser",
    "1× Rode lavalier mic",
    "1× Light",
    "2-day editing · Graphics extra/supplied by client",
    "Delivery of 1 Full HD video at D+2",
    "Original-language subtitles included"
  ],
  interviews: [
    "Filmed using kit already installed for the main event",
    "1× Lavalier mic included",
    "Up to 3 interviews",
    "Light editing included (cuts, joins, basic colour grading)",
    "Delivery D+1 in Full HD"
  ],
  photographe: [
    "1× Canon EOS 5D Mark IV or equivalent",
    "3× lenses",
    "Editing",
    "Delivery D+1/D+2 via weblink of 100+ photos"
  ]
};

// Technical kit details per option, shown in the collapsible "Tech view"
// when the option is ticked or forced by a partner code.
const OPTION_MATERIEL = {
  stream: [
    "Multi-RTMP setup (up to 5 simultaneous platforms)"
  ],
  son: [
    "1× Sound technician",
    "1× Multitrack mixer",
    "4× Wireless mics (handheld or headset)"
  ],
  lumiere: [
  ...
  reperage: [
    "On-site technical visit (half day)",
    "Filming plan tailored to the venue"
  ],
  duplex: [
    "Video conferencing software integrated into the gallery",
    "Latency < 200ms",
    "Speaker's audio routed via a dedicated speaker"
  ],
  veille: [
    "Set assembled the night before",
    "Full real-conditions tests",
    "Set ready for immediate start on day one",
    "Run-through on request"
  ],
  cam_sup: [
    "1× Panasonic CX350 4K camera (or equivalent)",
    "1× Camera tripod"
  ],
  rush4k: [
    "Native 4K files from each camera (XF-AVC / MP4 H.264)",
    "SSD drive handed over the same day"
  ],
  ecran: [
    "1× 24-inch confidence monitor on stand",
    "Dedicated video cable + power"
  ],
  "5g": [
    "1× Multi-carrier 5G aggregation router",
    "Cellular bonding (Orange + SFR + Bouygues + Free)",
    "Dedicated bandwidth up to 200 Mb/s upload"
  ],
  montage_tc: [
    "Files edited from the timecodes you provide",
    "Full HD MP4 H.264 export",
    "1 round of revisions included"
  ],
  cadreur_hf: [
    "1× Dedicated senior freelance camera operator",
    "1× Blackmagic 4K camera (or equivalent)",
    "1× HDMI/SDI wireless transmitter/receiver",
    "1× Camera tripod"
  ]
};

// Shows/hides the kit details directly under each ticked option (or forced by a partner).
// The block is inserted in the option line itself, just under the description, when the option is active.
  ...
  OPTIONS.forEach(opt => {
    ...
    if (isActive && OPTION_MATERIEL[opt.id]) {
      if (!materialBlock) {
        ...
        // Inserts after option-label (which contains label + desc + platforms)
        const labelEl = row.querySelector(".option-label");
        if (labelEl) labelEl.appendChild(materialBlock);
      }
      materialBlock.innerHTML = '<div class="option-material-title">Kit included</div>' +
...
function computeNaive(stateOverride) {
  ...
  if (s.addons.interviews) {
    ...
    addonLines.push({ id: "interviews", label: "Post-event interviews", price: ADDON_PRICES.interviews });
  }
  if (s.addons.photographe) {
    const price = ADDON_PRICES.photographe[s.duration] || ADDON_PRICES.photographe.half;
    addonsHT += price;
    addonLines.push({ id: "photographe", label: "Event photographer", price });
  }
  totalHT += addonsHT;
...
function render() {
  ...
  const suffix = state.ttc ? "incl. VAT" : "excl. VAT";
  const dayMult = getDayMult(state.duration);
  ...
  Object.keys(CONFIG.durations).forEach(key => {
    ...
    if (el) el.textContent = eur(shown(CONFIG.durations[key].price)) + " " + suffix;
  });
  ...
  if (r.lines.length) {
    ...
    r.lines.forEach(ln => {
      ...
      const valHTML = ln.isFree
        ? '<span class="val" style="color:var(--green);font-size:11px;letter-spacing:0.06em;text-transform:uppercase;">Included</span>'
        : (ln.oldPrice
            ? "<span class='val'><span class='price-old'>"+eur(shown(ln.oldPrice))+"</span><span class='price-new'>"+eur(shown(ln.price))+"</span></span>"
            : "<span class='val'>"+eur(shown(ln.price))+"</span>");
  ...
  if (partnerDiscSection && partnerDiscAmount) {
    if (r.partnerDiscount && r.partnerDiscount > 0) {
      ...
      partnerDiscAmount.textContent = "−" + eur(shown(r.partnerDiscount));
    } else {
  ...
  if (addonsSection && addonsContainer) {
    ...
    if (r.addonLines && r.addonLines.length) {
      ...
      r.addonLines.forEach(ln => {
        const row = document.createElement("div");
        row.className = "breakdown-line";
        row.innerHTML = "<span>" + ln.label + "</span><span class='val'>" + eur(shown(ln.price)) + "</span>";
  ...
  if (bestofPriceEl) {
    ...
    bestofPriceEl.textContent = "+ " + eur(shown(bestofPrice));
  }
  ...
  if (photographePriceEl) {
    ...
    photographePriceEl.textContent = "+ " + eur(shown(photographePrice));
  }
  ...
  if (setupTimeText) {
    setupTimeText.textContent = "On-site technical setup: " + getMontageH() + "h";
  }
  ...
  if (savingsBanner) {
    ...
    if (state.partnerCode && savings > 0) {
      savingsAmount.textContent = eur(shown(savings));
  ...
  // Total
  document.getElementById("total-label").textContent = "Total estimate " + suffix;
  document.getElementById("total-suffix").textContent = suffix;
  document.getElementById("mobile-label").textContent = "Estimate " + suffix;
  const newVal = eur(shown(r.totalHT));
  ...
  OPTIONS.forEach(opt => {
    ...
    if (isForcedByPartner || isDuplexFreeBySon || isAbsorbed || isAbsorbableTeasing) {
      ...
      priceEl.innerHTML = '<span class="free-badge">Free</span>';
    } else {
      ...
      if (opt.id === "son" && sonDuplex.sonPrice !== null) {
        ...
        priceEl.innerHTML = '<span class="price-old">' + eur(shown(fullPrice)) + '</span> <span class="price-new">+ ' + eur(shown(newP)) + '</span>';
      } else {
        priceEl.innerHTML = "+ " + eur(shown(fullPrice));
      }
    }
  });
...
function buildOptions() {
  ...
  OPTIONS.forEach(opt => {
    ...
    row.innerHTML = `
      ...
      <span class="option-price">+ ${eur(shown(opt.price))}</span>
    `;
...
function buildAddons() {
  ["bestof", "interviews", "photographe"].forEach(addonId => {
    ...
    matBlock.innerHTML = '<div class="addon-material-title">Service details</div>' +
    ...
    card.addEventListener("click", e => {
      ...
      const addonLabels = {
        ...
        interviews: "Post-event interviews",
        photographe: "Event photographer"
      };
      const addonPrice =
        addonId === "interviews"
...
function updatePartnerDisplay() {
  ...
  if (state.partnerCode) {
    ...
    if (txt) txt.textContent = "Active partner code · " + (state.partnerDisplayName || state.partnerCode);
    ...
    if (agencyBtn) {
      ...
      if (state.isAgence) {
        ...
        if (txtMain) txtMain.textContent = "I'm an event agency";
...
document.getElementById("agency-toggle").addEventListener("click", () => {
  ...
  if (state.isAgence) {
    txt.textContent = "White-label request";
    ...
    txt.textContent = "I'm an event agency";
...
function syncFormFields() {
  ...
  const optionLines = r.lines.map(o =>
    "  • " + o.label + " : " + (o.isFree ? "INCLUDED" : eur(o.price) + " (excl. VAT)")
  );
  const addonLines = (r.addonLines || []).map(a =>
    "  • " + a.label + " : " + eur(a.price) + " (excl. VAT)"
  );

  // Build the recap text message (read by envoyer.php into $message)
  const lines = [
    "── CONFIGURATOR SETUP ──",
  ...
  if (state.partnerCode) lines.push("Partner code: " + state.partnerCode);
  if (state.platforms.length) lines.push("Broadcast platforms: " + state.platforms.join(", "));
  if (optionLines.length) { lines.push("", "Selected options:"); lines.push.apply(lines, optionLines); }
  if (addonLines.length) { lines.push("", "Post-event content:"); lines.push.apply(lines, addonLines); }
  lines.push("", "Technical setup: " + mH + "h on site");
  lines.push("", "ESTIMATE: " + eur(r.totalHT) + " (excl. VAT) (" + eur(Math.round(r.totalHT * TVA)) + " incl. VAT)");
  if (state.isAgence) lines.push("", "→ WHITE-LABEL REQUEST (agency)");

  // User-visible fields already bound to the form via name=, only fill hidden fields here
  const set = (id, val) => { const el = document.getElementById(id); if (el) el.value = val; };
  set("h-type", eventLabel);
  set("h-message", lines.join("\n"));
  set("h-is-agence", state.isAgence ? "1" : "");
  set("h-cfg-duration", durLabel);
  set("h-cfg-options", r.lines.map(o => o.label + (o.isFree ? " (included)" : " (" + eur(o.price) + ")")).join(" | "));
  set("h-cfg-addons", (r.addonLines || []).map(a => a.label + " (" + eur(a.price) + ")").join(" | "));
  set("h-cfg-partner", state.partnerCode || "");
  set("h-cfg-total", eur(r.totalHT) + " (excl. VAT)");
...
// ═══ Tech view switch: drives detailed kit display (base set + under each ticked option/add-on) ═══
// Toggles body.classList "tech-mode" which reveals:
//   - <div id="tech-base-details"> block (base set kit)
//   - <div class="option-material"> under each ticked option (CSS rule .tech-mode .option.active .option-material)
//   - <div class="addon-material"> under each ticked add-on (CSS rule .tech-mode .addon-card.active .addon-material)
// Also drives .active class on the Tech view banner for visual feedback (cyan border, cyan slider).
